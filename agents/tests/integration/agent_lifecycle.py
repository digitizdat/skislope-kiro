...
from .models import AgentHealthStatus, TestError, Severity
...
class AgentLifecycleManager:
    ...
    def __init__(self, config: TestConfig, logger: Optional[TestLogger] = None):
        """
    ...
    async def _check_agent_health(self, agent: AgentProcess) -> Optional[AgentHealthStatus]:
        """
    ...
    async def start_all_agents(self, parallel: bool = True) -> Tuple[List[str], List[str]]:
        """
        ...
        """
        enabled_agents = [name for name, agent in self.agents.items() 
        ...
        if parallel:
            ...
            for agent_name, result in zip(enabled_agents, results):
            ...
            for agent_name in enabled_agents:
                if await self.start_agent(agent_name, max_attempts=self.config.retry_attempts):
                    successful.append(agent_name)
                else:
                    failed.append(agent_name)
    ...
    async def stop_agent(self, agent_name: str, timeout: float = None) -> bool:
        """
    ...
    async def stop_all_agents(self, timeout: float = None) -> List[str]:
        """
        ...
        self.logger.info(f"Stopping {len(running_agents)} agents", agents=running_agents)
        
        # Stop all agents in parallel
        tasks = [
            self.stop_agent(agent_name, timeout)
        ...
        failed_to_stop = []
        for agent_name, result in zip(running_agents, results):
            if isinstance(result, Exception) or not result:
                failed_to_stop.append(agent_name)
        
        if failed_to_stop:
            self.logger.warning(
                f"Some agents failed to stop cleanly",
                failed_agents=failed_to_stop
            )
        else:
    ...
    async def wait_for_agents_ready(
        ...
        self.logger.info(
            f"Waiting for agents to become ready",
            agents=agent_names,
            timeout=timeout
        )
        ...
        self.logger.warning(
            f"Timeout waiting for agents to become ready",
            timeout=timeout,
            ready=ready_agents,
    ...
    def are_all_agents_healthy(self, agent_names: Optional[List[str]] = None) -> bool:
