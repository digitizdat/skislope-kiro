...
async def main():
    ...
    config = get_test_config({
        "timeouts.agent_startup": 30,
        "timeouts.shutdown": 10,
        "log_level": "INFO"
    })
    ...
    async with AgentLifecycleManager(config, logger) as manager:
        ...
        if not successful:
            print("No agents started successfully. This is expected if agent servers are not available.")
            return
        ...
        for agent_name, status in health_status.items():
            ...
            print(f"{health_indicator} {agent_name}: {status.status} "
