...
from agents.hill_metrics.models import GridSize, SurfaceType
from agents.weather.models import WeatherCondition, SnowCondition
from agents.equipment.models import LiftStatus, TrailStatus
...
class TestMockDataGenerator:
    ...
    def test_scenario_extreme_weather_terrain(self):
        ...
        ice_count = sum(
            1 for row in surface_grid 
            for cell in row 
            if cell == SurfaceType.ICE
        )
    ...
    def test_scenario_equipment_failures(self):
        ...
        # Count failed lifts
        failed_lifts = sum(
            1 for lift in response.lifts 
            if lift.status != LiftStatus.OPERATIONAL
        )
        
        # Count closed trails
        closed_trails = sum(
            1 for trail in response.trails 
    ...
    def test_reproducible_generation_with_seed(self):
        ...
        assert len(response1.hill_metrics.elevation.grid) == len(response2.hill_metrics.elevation.grid)
        assert len(response1.hill_metrics.elevation.grid[0]) == len(response2.hill_metrics.elevation.grid[0])
        ...
        for i in range(min(5, len(grid1[0]))):
            assert abs(grid1[0][i] - grid2[0][i]) < 1.0, f"Values differ too much: {grid1[0][i]} vs {grid2[0][i]}"
    ...
    def test_different_generation_with_different_seeds(self):
        ...
        assert response1.hill_metrics.elevation.grid != response2.hill_metrics.elevation.grid
    ...
    def test_performance_load_factor_affects_processing_time(self):
        ...
        assert response_high_load.processing_time_ms > response_normal.processing_time_ms
