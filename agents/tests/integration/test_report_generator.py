...
class TestReportGenerator:
    ...
    def test_generate_junit_report(
        self, report_generator, sample_test_results, temp_dir
    ):
    ...
    def test_generate_multiple_formats(
        ...
        for _format_type, path in report_paths.items():
            assert path.exists()
            assert path.parent == temp_dir

    def test_generate_report_with_compression(
        self, report_generator, sample_test_results, temp_dir
    ):
    ...
    def test_generate_report_auto_path(self, report_generator, sample_test_results):
        ...
        result_path = report_generator.generate_report(
            results=sample_test_results, format_type=ReportFormat.JSON
        )
        ...
        try:
            ...
            # Directory not empty, which is fine - other tests might be using it
            pass
    ...
    def test_html_sections_generation(self, report_generator, sample_test_results):
        ...
        category_html = report_generator._generate_category_section_html(
            sample_test_results
        )
        assert "Test Categories" in category_html
        assert "api contracts" in category_html.lower()
        assert "category-card" in category_html

        # Test agent health section
        agent_html = report_generator._generate_agent_health_section_html(
            sample_test_results
        )
        ...
        env_html = report_generator._generate_environment_section_html(
            sample_test_results
        )
        ...
        perf_html = report_generator._generate_performance_section_html(
            sample_test_results
        )
        assert "Performance Metrics" in perf_html
        assert "45.5" in perf_html  # total duration

    def test_markdown_emoji_usage(
        self, report_generator, sample_test_results, temp_dir
    ):
    ...
    def test_report_generation_with_config(self, mock_browser, temp_dir):
        ...
        result_path = report_generator.generate_report(
            results=results, format_type=ReportFormat.HTML, output_path=output_path
        )
