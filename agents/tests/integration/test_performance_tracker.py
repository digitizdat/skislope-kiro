...
from .performance_tracker import PerformanceWindow
from .performance_tracker import ResourceSnapshot
...
class TestPerformanceTracker:
    ...
    def test_analyze_trends_insufficient_data(self, tracker, mock_psutil):
        ...
        if len(tracker.windows["minimal_test"].snapshots) > 1:
            tracker.windows["minimal_test"].snapshots = tracker.windows[
                "minimal_test"
            ].snapshots[:1]
    ...
    def test_tracking_thread_error_handling(self, tracker):
        ...
        with patch(
            "agents.tests.integration.performance_tracker.psutil.cpu_percent",
    ...
    @patch(
        "agents.tests.integration.performance_tracker.psutil.disk_io_counters",
        return_value=None,
    )
