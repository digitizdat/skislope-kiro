...
from .logging_utils import DiagnosticCollector
from .logging_utils import TestArtifactManager
from .logging_utils import TestLogger
from .logging_utils import integration_test_context
...
class TestConfigManager:
    ...
    def test_config_overrides(self):
        ...
        overrides = {"max_workers": 8, "timeouts.agent_startup": 60}
...
class TestDataModels:
    ...
    def test_test_results_aggregation(self):
        ...
        result1 = TestResult(
            "test1", TestCategory.API_CONTRACTS, TestStatus.PASSED, 1.0
        )
        result1.mark_passed()

        result2 = TestResult(
            "test2", TestCategory.API_CONTRACTS, TestStatus.FAILED, 2.0
        )
        result2.mark_failed("Test failed")

        result3 = TestResult(
            "test3", TestCategory.COMMUNICATION, TestStatus.PASSED, 1.5
        )
        result3.mark_passed()
...
class TestLoggingUtils:
    ...
    def test_artifact_manager(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            config = get_test_config({"env_config.temp_dir": Path(temp_dir)})
