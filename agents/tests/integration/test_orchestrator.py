...
class IntegrationTestOrchestrator:
    ...
    def __init__(self, config: TestConfig | None = None):
        ...
        self.agent_manager: AgentLifecycleManager | None = None
        self.api_validator: APIContractValidator | None = None
        self.env_validator: EnvironmentValidator | None = None
        self.comm_validator: CommunicationValidator | None = None
        self.workflow_simulator: WorkflowSimulator | None = None

        # Reporting and monitoring components
        self.report_generator: ReportGenerator | None = None
        self.dashboard: TestResultsDashboard | None = None
        self.performance_tracker: PerformanceTracker | None = None

        # Execution state
        self.execution_context: ExecutionContext | None = None
        self.results: TestResults | None = None
        self._shutdown_requested = False

        # Test category handlers
        self.category_handlers: dict[
            TestCategory, Callable[[ExecutionContext], Awaitable[list[TestResult]]]
        ] = {
    ...
    async def _initialize_components(self) -> None:
        ...
        self.workflow_simulator = WorkflowSimulator(
            self.config, self.agent_manager, self.logger
        )
        ...
        self.performance_tracker = PerformanceTracker(
            sampling_interval=1.0, max_samples=1000
        )
    ...
    async def run_selective_tests(
        self, categories: list[TestCategory], context: dict[str, Any] | None = None
    ) -> TestResults:
        """
        ...
        try:
            ...
            if self.performance_tracker:
                self.results.performance_metrics = (
                    self.performance_tracker.create_performance_metrics()
                )
    ...
    async def _pre_execution_setup(self) -> None:
        ...
        if any(
            ...
            for cat in self.execution_context.config.agents
        ):
            self.logger.info("Starting agent servers")
            (
                successful_agents,
                failed_agents,
            ) = await self.agent_manager.start_all_agents(parallel=True)
            ...
            if successful_agents:
                ready = await self.agent_manager.wait_for_agents_ready(
                    successful_agents, timeout=self.config.timeouts.agent_startup
                )
    ...
    async def _post_execution_cleanup(self) -> None:
        ...
        if self.agent_manager:
            ...
            if failed_to_stop:
                self.logger.warning(
                    f"Some agents failed to stop cleanly: {failed_to_stop}"
                )
    ...
    async def _execute_parallel(self, plan: ExecutionPlan) -> None:
        ...
        self.logger.info(
            f"Executing {len(plan.categories)} test categories in parallel"
        )

        # Create semaphore to limit concurrent workers
        semaphore = asyncio.Semaphore(plan.max_workers)

        async def run_category_with_semaphore(
        ...
        tasks = [run_category_with_semaphore(category) for category in plan.categories]

        try:
            # Wait for all tasks with timeout
            category_results = await asyncio.wait_for(
                asyncio.gather(*tasks, return_exceptions=True), timeout=plan.timeout
            )
            ...
            for i, result in enumerate(category_results):
                ...
                if isinstance(result, Exception):
                    ...
                    error_result.mark_failed(
                        TestError.from_exception(
                            result, category.value, Severity.CRITICAL
                        )
                    )
                    self.results.add_result(error_result)
                else:
    ...
    async def _execute_sequential(self, plan: ExecutionPlan) -> None:
        ...
        self.logger.info(
            f"Executing {len(plan.categories)} test categories sequentially"
        )
            ...
            try:
                ...
                timeout_result.mark_failed(
                    TestError(
                        ...
                        message="Category tests timed out",
    ...
    async def _run_environment_tests(
    ...
    async def _run_api_contract_tests(
        self, context: ExecutionContext
    ) -> list[TestResult]:
    ...
    async def _run_communication_tests(
        self, context: ExecutionContext
    ) -> list[TestResult]:
        ...
        try:
            ...
            for validation_result in validation_results:
                ...
                if validation_result.overall_valid:
                    ...
                    error_messages = []
                    if (
                        validation_result.cors_result
                        and not validation_result.cors_result.is_valid
                    ):
                        error_messages.append(
                            f"CORS issues: {len(validation_result.cors_result.cors_violations)}"
                        )
                    if (
                        validation_result.jsonrpc_result
                        and not validation_result.jsonrpc_result.is_valid
                    ):
                        error_messages.append(
                            f"JSON-RPC issues: {len(validation_result.jsonrpc_result.protocol_violations)}"
                        )
                    if (
                        validation_result.network_result
                        and not validation_result.network_result.is_valid
                    ):
                        error_messages.append(
                            f"Network issues: {len(validation_result.network_result.network_issues)}"
                        )
        ...
        try:
            ...
            summary_data = {
                ...
                "agent_health_summary": {
                    agent.name: agent.status for agent in self.results.agent_health
                },
    ...
    async def _collect_system_diagnostics(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to collect system diagnostics: {e}", exc_info=True
            )

    def start_dashboard(
        self, port: int = 8080, open_browser: bool = True
    ) -> str | None:
        """
    ...
    async def _save_results(self) -> None:
        ...
        try:
            ...
            if failed_tests:
                ...
                with open(failed_file, "w") as f:
                    json.dump(
                        [test.to_dict() for test in failed_tests],
                        f,
                        indent=2,
                        default=str,
                    )
