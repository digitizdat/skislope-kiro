...
from agents.tests.integration.models import TestError
...
class CommunicationValidationResult:
    ...
    agent_name: str
    cors_result: CORSTestResult | None = None
    jsonrpc_result: JSONRPCTestResult | None = None
    mcp_result: MCPTestResult | None = None
    network_result: NetworkTestResult | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "jsonrpc_result": self.jsonrpc_result.to_dict()
            if self.jsonrpc_result
            else None,
            "mcp_result": self.mcp_result.to_dict() if self.mcp_result else None,
            "network_result": self.network_result.to_dict()
...
class CommunicationValidator:
    ...
    async def validate_all_communication(self) -> list[CommunicationValidationResult]:
        ...
        for agent_name, agent_config in self.config.agents.items():
            ...
            try:
                result = await self.validate_agent_communication(
                ...
                result = CommunicationValidationResult(
                    ...
                    overall_valid=False,
                    validation_errors=[f"Communication validation failed: {e!s}"],
                )
                results.append(result)
    ...
    async def validate_agent_communication(
        self, agent_name: str, agent_config: AgentConfig
        ...
        try:
            ...
            result.cors_result = await self._test_cors_configuration(
                agent_name, agent_config
            )

            # Test JSON-RPC protocol compliance
            result.jsonrpc_result = await self._test_jsonrpc_protocol(
                agent_name, agent_config
            )
            ...
            result.network_result = await self._test_network_mechanisms(
                agent_name, agent_config
            )
            ...
            logger.info(
                ...
                jsonrpc_valid=result.jsonrpc_result.is_valid
                if result.jsonrpc_result
                else None,
                mcp_valid=result.mcp_result.is_valid if result.mcp_result else None,
                network_valid=result.network_result.is_valid
        ...
        try:
            ...
            if result.allowed_origins:
                ...
                async with self.session.post(
                    endpoint, json=test_request, headers=headers
                ) as response:
                    cors_origin = response.headers.get("Access-Control-Allow-Origin")
                    if not cors_origin or (
                        cors_origin != "*" and cors_origin != test_origin
                    ):
                        result.cors_violations.append(
            ...
            result.is_valid = (
                len(result.cors_violations) == 0 and result.preflight_successful
            )

        except Exception as e:
            result.cors_violations.append(f"CORS test failed: {e!s}")
        ...
        try:
            ...
            async with self.session.post(endpoint, json=valid_request) as response:
                if response.status == 200:
                    ...
                    if self._validate_jsonrpc_response(
                        response_data, valid_request["id"]
                    ):
            ...
            for test in error_tests:
                try:
                        ...
                        async with self.session.post(
                            ...
                            if response.status == 200:
                                ...
                                if not self._validate_jsonrpc_error(
                                    response_data, test["expected_code"]
                                ):
                                    result.protocol_violations.append(
                    ...
                    result.protocol_violations.append(
                        f"{test['description']} test failed: {e!s}"
                    )
                    error_handling_valid = False
            ...
            batch_request = [
                {"jsonrpc": "2.0", "method": "healthCheck", "params": {}, "id": 1},
                {"jsonrpc": "2.0", "method": "healthCheck", "params": {}, "id": 2},
            ]
            ...
            try:
                async with self.session.post(
                    endpoint, json=notification_request
                ) as response:
            ...
            result.is_valid = (
                result.request_response_valid and result.error_handling_valid
            )

        except Exception as e:
            result.protocol_violations.append(f"JSON-RPC protocol test failed: {e!s}")
            result.is_valid = False
    ...
    async def _test_mcp_protocol(
        ...
        try:
            ...
            async with self.session.get(endpoint) as response:
                if response.status == 200:
                    ...
                    result.mcp_violations.append(
                        f"MCP endpoint not available: HTTP {response.status}"
                    )
            ...
            if result.available_tools:
                ...
                tool_request = {"tool": test_tool, "parameters": {}}

                try:
                    async with self.session.post(
                        f"{endpoint}/execute", json=tool_request
                    ) as response:
                        if response.status == 200:
                    ...
                    result.mcp_violations.append(f"Tool execution test failed: {e!s}")

            result.is_valid = (
                not result.mcp_supported  # If MCP not supported, that's okay
                or (result.tool_discovery_valid and result.tool_execution_valid)
            )

        except Exception as e:
            # MCP support is optional, so failures here don't invalidate the agent
            result.mcp_supported = False
            result.mcp_violations.append(f"MCP test failed: {e!s}")
    ...
    async def _test_network_mechanisms(
        ...
        try:
            ...
            try:
                ...
                result.timeout_handling_valid = True
                result.performance_metrics["timeout_duration"] = (
                    time.time() - start_time
                )
            except Exception as e:
                result.network_issues.append(f"Timeout test failed: {e!s}")
            ...
            for attempt in range(retry_attempts):
                try:
                    ...
                    result.network_issues.append(
                        f"Retry attempt {attempt + 1} failed: {e!s}"
                    )

            result.retry_mechanism_valid = retry_success_count >= (
                retry_attempts * 0.7
            )  # 70% success rate
            result.performance_metrics["retry_success_rate"] = (
                retry_success_count / retry_attempts
            )
            ...
            successful_concurrent = sum(
                1 for r in concurrent_results if not isinstance(r, Exception) and r
            )

            result.concurrent_request_handling = successful_concurrent >= (
                concurrent_requests * 0.8
            )
            result.performance_metrics["concurrent_success_rate"] = (
            ...
            try:
                start_time = time.time()
                async with self.session.post(
                    endpoint, json=large_payload_request
                ) as response:
                    if response.status == 200:
                        result.large_payload_handling = True
                        result.performance_metrics["large_payload_response_time"] = (
                            time.time() - start_time
                        )
                    else:
                        result.network_issues.append(
                            f"Large payload failed: HTTP {response.status}"
                        )
                        result.large_payload_handling = False
            except Exception as e:
                result.network_issues.append(f"Large payload test failed: {e!s}")
                result.large_payload_handling = False

            result.is_valid = (
                result.timeout_handling_valid
                and result.retry_mechanism_valid
                and result.concurrent_request_handling
                and result.large_payload_handling
            )

        except Exception as e:
            result.network_issues.append(f"Network mechanism test failed: {e!s}")
            result.is_valid = False

        return result

    async def _make_concurrent_request(
    ...
    def _validate_jsonrpc_response(
        self, response_data: dict[str, Any], expected_id: Any
    ) -> bool:
        """
        ...
        return response_data.get("id") == expected_id

    def _validate_jsonrpc_error(
        self, response_data: dict[str, Any], expected_code: int
    ) -> bool:
        """
        ...
        return not ("message" not in error or not isinstance(error["message"], str))
