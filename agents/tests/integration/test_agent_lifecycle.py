...
from .agent_lifecycle import AgentLifecycleManager, AgentProcess, AgentState
from .config import TestConfig, AgentConfig, TimeoutConfig
...
class TestAgentLifecycleManager:
    ...
    def test_get_agent_module_path(self, lifecycle_manager):
        ...
        assert lifecycle_manager._get_agent_module_path("hill_metrics") == "agents.hill_metrics.server"
        assert lifecycle_manager._get_agent_module_path("weather") == "agents.weather.server"
        assert lifecycle_manager._get_agent_module_path("equipment") == "agents.equipment.server"
        
        # Test unknown agent (should construct generic path for testing)
        assert lifecycle_manager._get_agent_module_path("unknown_agent") == "agents.unknown_agent.server"
    ...
    async def test_check_agent_health_success(self, lifecycle_manager):
        ...
        methods_response.json.return_value = {
            "result": ["test_method", "other_method"]
        }
        mock_client.post.return_value = methods_response
        
        with patch('time.time', side_effect=[1000.0, 1000.1, 1000.2]):  # Multiple calls to time.time()
    ...
    async def test_start_agent_failure(self, lifecycle_manager):
        ...
        with patch('subprocess.Popen') as mock_popen, \
             patch.object(lifecycle_manager, '_wait_for_agent_ready', return_value=False), \
    ...
    async def test_start_all_agents_parallel(self, lifecycle_manager):
        ...
        with patch.object(lifecycle_manager, 'start_agent', return_value=True) as mock_start:
    ...
    async def test_start_all_agents_sequential(self, lifecycle_manager):
        ...
        with patch.object(lifecycle_manager, 'start_agent', return_value=True) as mock_start:
            successful, failed = await lifecycle_manager.start_all_agents(parallel=False)
    ...
    async def test_start_all_agents_with_failures(self, lifecycle_manager):
        ...
        with patch.object(lifecycle_manager, 'start_agent', return_value=False) as mock_start:
    ...
    async def test_stop_all_agents(self, lifecycle_manager):
        ...
        with patch.object(lifecycle_manager, 'stop_agent', return_value=True) as mock_stop:
    ...
    async def test_restart_agent(self, lifecycle_manager):
        """Test agent restart."""
        with patch.object(lifecycle_manager, 'stop_agent', return_value=True) as mock_stop, \
             patch.object(lifecycle_manager, 'start_agent', return_value=True) as mock_start:
    ...
    async def test_check_all_agents_health(self, lifecycle_manager):
        ...
        with patch.object(lifecycle_manager, '_check_agent_health', return_value=healthy_status):
    ...
    async def test_wait_for_agents_ready_success(self, lifecycle_manager):
        ...
        with patch.object(lifecycle_manager, 'check_all_agents_health', 
    ...
    def test_get_all_agent_status(self, lifecycle_manager):
        ...
        with patch.object(lifecycle_manager, 'get_agent_status', return_value={"test": "status"}):
