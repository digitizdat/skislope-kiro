...
from agents.tests.integration.api_contract_validator import ContractValidationResult
from agents.tests.integration.api_contract_validator import MethodSignature
...
class TestAPIContractValidator:
    ...
    def test_config(self):
        ...
        config.agents = {
            ...
            "weather": AgentConfig(
                name="weather", host="localhost", port=8002, enabled=True
            ),
        }
    ...
    def test_compare_method_signatures_missing_backend_param(self, validator):
        ...
        backend_method = MethodSignature(
            name="test_method", parameters={"param1": {"type": "str", "required": True}}
        )
    ...
    def test_compare_method_signatures_type_mismatch(self, validator):
        ...
        backend_method = MethodSignature(
            name="test_method",
            parameters={"param1": {"type": "CustomType", "required": True}},
        )

        # Mock type compatibility to return False
        with patch.object(validator, "_are_types_compatible", return_value=False):
            result = validator._compare_method_signatures(
                frontend_method, backend_method
            )
            assert result is not None
    ...
    def test_is_valid_jsonrpc_response_valid(self, validator):
        ...
        valid_response = {"jsonrpc": "2.0", "result": {"data": "test"}, "id": 1}
    ...
    async def test_run_contract_tests_success(self, validator):
        ...
        mock_result = ContractValidationResult(agent_name="test_agent", is_valid=True)

        with patch.object(
            validator, "validate_all_contracts", return_value=[mock_result]
        ):
    ...
    async def test_run_contract_tests_exception(self, validator):
        ...
        with patch.object(
            validator, "validate_all_contracts", side_effect=Exception("Test error")
        ):
            test_results = await validator.run_contract_tests()

        assert len(test_results) == 1
        assert (
            test_results[0].status == TestStatus.FAILED
        )  # Changed from ERROR to FAILED
        assert test_results[0].error is not None
    ...
    async def test_jsonrpc_protocol_validation_no_session(self):
        ...
        config.agents = {
            "test_agent": AgentConfig(
                name="test_agent", host="localhost", port=8999, enabled=True
            )
        }
        ...
        await validator._validate_jsonrpc_compliance(
            "test_agent", config.agents["test_agent"], result
        )
