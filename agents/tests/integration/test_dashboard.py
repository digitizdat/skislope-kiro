...
from .dashboard import DashboardData, DashboardServer, TestResultsDashboard
from .models import (
    TestResults, TestResult, TestCategory, TestStatus, TestSummary,
    CategoryResults, PerformanceMetrics, AgentHealthStatus
)
...
class TestDashboardData:
    ...
    def test_historical_summary(self, dashboard_data):
        ...
        for rate in success_rates:
            ...
            results.summary = TestSummary(
                total_tests=10,
                passed=int(10 * rate / 100),
                failed=10 - int(10 * rate / 100),
                duration=30.0,
                start_time=datetime.now()
            )
            dashboard_data.add_results(results)
        ...
        assert summary["average_success_rate"] == sum(success_rates) / len(success_rates)
...
class TestDashboardServer:
    ...
    def test_server_start_stop(self, mock_browser, dashboard_server):
        ...
        assert dashboard_server.server is None or not dashboard_server.server_thread.is_alive()
...
class TestTestResultsDashboard:
    ...
    @patch('agents.tests.integration.dashboard.ReportGenerator')
    def test_report_generator_integration(self, mock_report_gen, dashboard, sample_results):
    ...
    def test_dashboard_data_persistence(self, dashboard):
        ...
        url = dashboard.start_server(port=0, open_browser=False)
        dashboard.stop_server()
