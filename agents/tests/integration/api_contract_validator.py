...
class APIContractValidator:
        ...
        for agent_name, agent_config in self.config.agents.items():
            ...
            try:
                ...
                result = ContractValidationResult(
                    ...
                    is_valid=False,
                    protocol_violations=[f"Contract validation failed: {e!s}"],
                )
                results.append(result)
    ...
    async def validate_agent_contract(
        ...
        try:
            ...
            result.is_valid = (
                len(result.missing_backend_methods) == 0
                and len(result.signature_mismatches) == 0
                and len(result.protocol_violations) == 0
            )
            ...
            result.protocol_violations.append(f"Validation error: {e!s}")
    ...
    async def _discover_backend_methods(self, agent_name: str) -> list[MethodSignature]:
        ...
        try:
            ...
            if hasattr(module, "jsonrpc_handler") and hasattr(
                module.jsonrpc_handler, "methods"
            ):
    ...
    def _get_frontend_methods(self, agent_name: str) -> list[MethodSignature]:
        ...
        if agent_name in self.frontend_method_mappings:
            for method_name, method_info in self.frontend_method_mappings[
                agent_name
            ].items():
    ...
    def _compare_methods(self, result: ContractValidationResult) -> None:
        ...
        result.missing_backend_methods = list(
            frontend_method_names - backend_method_names
        )
        result.missing_frontend_methods = list(
            backend_method_names - frontend_method_names
        )
        ...
        for method_name in common_methods:
            frontend_method = next(
                m for m in result.frontend_methods if m.name == method_name
            )
            backend_method = next(
                m for m in result.backend_methods if m.name == method_name
            )
    ...
    def _compare_method_signatures(
        self, frontend_method: MethodSignature, backend_method: MethodSignature
    ) -> dict[str, Any] | None:
        """
        ...
        if extra_backend_params:
            ...
            if extra_required:
                mismatches.append(
                    f"Extra required backend parameters: {extra_required}"
                )
        ...
        for param in common_params:
            ...
            if not self._are_types_compatible(
                frontend_param.get("type"), backend_param.get("type")
            ):
                mismatches.append(
    ...
    async def _validate_jsonrpc_compliance(
        ...
        if not self.session:
            result.protocol_violations.append(
                "No HTTP session available for protocol testing"
            )
            return
        ...
        try:
            ...
            valid_request = {
                "jsonrpc": "2.0",
                "method": "healthCheck"
                if "healthCheck" in [m.name for m in result.backend_methods]
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    response_data = await response.json()
                    if (
                        not response_data.get("error")
                        or response_data["error"].get("code") != -32700
                    ):
                        result.protocol_violations.append(
            ...
            async with self.session.post(endpoint, json=not_found_request) as response:
                if response.status == 200:
                    ...
                    if (
                        not response_data.get("error")
                        or response_data["error"].get("code") != -32601
                    ):
                        result.protocol_violations.append(
            ...
            result.protocol_violations.append(f"Protocol compliance test failed: {e!s}")
    ...
    def discover_agent_methods_static(
        self, agent_file_path: str
    ...
    def _generate_suggested_fix(
        self, validation_result: ContractValidationResult
    ) -> str:
        """
