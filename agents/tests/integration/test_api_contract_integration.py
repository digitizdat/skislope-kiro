    ...
    async def test_discover_hill_metrics_methods(self, test_config):
        ...
        try:
            ...
            for expected_method in expected_methods:
                assert expected_method in method_names, f"Expected method {expected_method} not found in {method_names}"
            
            # Check method signatures
            hill_metrics_method = next((m for m in methods if m.name == "getHillMetrics"), None)
    ...
    async def test_validate_hill_metrics_contract(self, test_config):
        ...
        try:
            ...
            result = await validator.validate_agent_contract(
                "hill_metrics", 
                test_config.agents["hill_metrics"]
            )
            ...
            if not result.missing_backend_methods and not result.signature_mismatches:
                ...
                print(f"Contract validation successful - methods match")
                print(f"Protocol violations (expected in test): {result.protocol_violations}")
            else:
                ...
                if result.missing_backend_methods:
                    assert False, f"Missing backend methods: {result.missing_backend_methods}"
