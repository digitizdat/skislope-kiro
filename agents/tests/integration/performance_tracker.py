...
class PerformanceWindow:
    ...
    def requests_per_second(self) -> float:
        ...
        if self.duration > 0:
            ...
            total_requests = (self.network_bytes_sent + self.network_bytes_recv) / 1024  # KB as proxy
...
class PerformanceTracker:
    ...
    def __init__(self, sampling_interval: float = 1.0, max_samples: int = 1000):
        ...
        self.initial_network_stats: Optional[Dict[str, int]] = None
    ...
    def stop_tracking(self) -> PerformanceWindow:
        ...
        self.logger.info(f"Stopping performance tracking for window: {self.current_window}")
    ...
    def end_phase(self) -> Optional[PerformanceWindow]:
        """
    ...
    def get_current_metrics(self) -> Optional[ResourceSnapshot]:
        """
    ...
    def create_performance_metrics(self) -> PerformanceMetrics:
        ...
        setup_duration = self.windows.get("setup", self.windows.get("environment", None))
        execution_duration = self.windows.get("execution", self.windows.get("tests", None))
        teardown_duration = self.windows.get("teardown", self.windows.get("cleanup", None))
        ...
        return PerformanceMetrics(
            ...
            execution_duration=execution_duration.duration if execution_duration else total_duration,
    ...
    def analyze_trends(self, window_name: str) -> Dict[str, Any]:
        ...
        network_sent_values = [s.network_bytes_sent for s in window.snapshots]
        network_recv_values = [s.network_bytes_recv for s in window.snapshots]
        ...
        return {
            ...
            "cpu_trend": {
                ...
                "variance": statistics.variance(cpu_values) if len(cpu_values) > 1 else 0
            },
            "memory_trend": {
    ...
    def _create_performance_window(self) -> PerformanceWindow:
        ...
        if self.samples and self.initial_network_stats:
            ...
            network_sent = final_snapshot.network_bytes_sent - self.initial_network_stats["bytes_sent"]
            network_recv = final_snapshot.network_bytes_recv - self.initial_network_stats["bytes_recv"]
        else:
        ...
        if self.samples and self.baseline_snapshot:
            final_snapshot = self.samples[-1]
            disk_read = final_snapshot.disk_io_read - self.baseline_snapshot.disk_io_read
            disk_write = final_snapshot.disk_io_write - self.baseline_snapshot.disk_io_write
        else:
