...
from .models import EnvironmentIssue, Severity, TestError
...
class PackageInfo:
    ...
    version: str
    location: str
    installer: Optional[str] = None  # 'uv', 'pip', 'conda', etc.
...
class EnvironmentValidator:
    ...
    def __init__(self, config: Optional[TestConfig] = None):
        ...
        self.required_packages = {
            'fastapi', 'uvicorn', 'structlog', 'rasterio', 'shapely',
            'numpy', 'requests', 'aiohttp', 'pytest'
        }
    ...
    def validate_environment(self) -> EnvironmentValidationResult:
        ...
        is_valid = not any(issue.severity in [Severity.CRITICAL, Severity.HIGH] for issue in self.issues)
    ...
    def _validate_python_environment(self) -> None:
        ...
        if sys.version_info < (3, 8):
            self.issues.append(EnvironmentIssue(
                component="python",
                issue_type="version",
                description="Python version is too old",
                severity=Severity.CRITICAL,
                detected_value=f"{sys.version_info[0]}.{sys.version_info[1]}",
                expected_value=">=3.8",
                suggested_fix="Upgrade to Python 3.8 or newer"
            ))
            ...
            try:
                with urllib.request.urlopen(url, timeout=10) as response:
                    if response.status != 200:
                        raise urllib.error.HTTPError(url, response.status, "Non-200 response", None, None)
