...
from .models import (
    TestResults, TestResult, TestCategory, TestStatus, Severity,
    AgentHealthStatus, EnvironmentIssue, PerformanceMetrics
)
...
class ReportGenerator:
    ...
    def generate_report(
        ...
        # Write content
        if compress and format_type in [ReportFormat.JSON, ReportFormat.HTML, ReportFormat.MARKDOWN]:
    ...
    def _generate_json_report(self, results: TestResults, include_diagnostics: bool) -> str:
        ...
        report_data = {
            ...
            "agent_health": [
                agent.to_dict() if hasattr(agent, 'to_dict') else self._agent_to_dict(agent)
    ...
    def _generate_html_report(self, results: TestResults, include_diagnostics: bool) -> str:
...
"""
        ...
        for category, category_results in results.categories.items():
            ...
            for test in category_results.tests:
                ...
                if test.status == TestStatus.FAILED:
                    ...
                    failure.set("message", test.error.message if test.error else "Test failed")
                    if test.error and test.error.stack_trace:
                        failure.text = test.error.stack_trace
                elif test.status == TestStatus.ERROR:
                    error = ET.SubElement(testcase, "error")
                    error.set("message", test.error.message if test.error else "Test error")
                    if test.error and test.error.stack_trace:
    ...
    def _generate_markdown_report(self, results: TestResults, include_diagnostics: bool) -> str:
...
"""
        ...
        for category, category_results in results.categories.items():
            category_emoji = "✅" if category_results.success_rate >= 90 else "⚠️" if category_results.success_rate >= 70 else "❌"
...
"""
        ...
        if results.agent_health:
            ...
            for agent in results.agent_health:
                status_emoji = "✅" if agent.status == "healthy" else "⚠️" if agent.status == "degraded" else "❌"
        ...
        if results.environment_issues:
            ...
            for issue in results.environment_issues:
                ...
                md_content += f"- {severity_emoji} **{issue.component}**: {issue.description}\n"
            md_content += "\n"
...
"""
    ...
    def _generate_agent_health_section_html(self, results: TestResults) -> str:
        ...
        for agent in results.agent_health:
            status_class = "healthy" if agent.status == "healthy" else "degraded" if agent.status == "degraded" else "failed"
