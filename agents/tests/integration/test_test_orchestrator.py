...
from agents.tests.integration.test_orchestrator import (
    IntegrationTestOrchestrator, ExecutionMode, ExecutionPlan
)
...
from agents.tests.integration.models import (
    TestResult, TestResults, TestCategory, TestStatus, TestError, Severity,
    AgentHealthStatus
)
...
class TestIntegrationTestOrchestrator:
    ...
    async def test_run_smoke_tests(self, test_config):
        ...
        with patch.object(orchestrator, '_execute_plan') as mock_execute:
            ...
            assert call_args.categories == {TestCategory.ENVIRONMENT, TestCategory.API_CONTRACTS}
    ...
    async def test_run_performance_tests(self, test_config):
        ...
        with patch.object(orchestrator, '_execute_plan') as mock_execute:
            ...
            assert not call_args.parallel_execution  # Performance tests run sequentially
    ...
    async def test_parallel_execution_timeout(self, test_config):
        ...
        with patch.object(orchestrator, '_run_category_tests', side_effect=slow_category_test):
            plan = ExecutionPlan(
                mode=ExecutionMode.SELECTIVE,
                categories={TestCategory.ENVIRONMENT},
                parallel_execution=True,
    ...
    async def test_api_contract_tests(self, test_config):
        ...
        mock_test_results = [
            TestResult("API Test 1", TestCategory.API_CONTRACTS, TestStatus.PASSED, 1.0),
            TestResult("API Test 2", TestCategory.API_CONTRACTS, TestStatus.FAILED, 2.0)
        ]
    ...
    async def test_save_results(self, test_config):
        ...
        # Add a failed test
        failed_test = TestResult("Failed Test", TestCategory.ENVIRONMENT, TestStatus.FAILED, 1.0)
