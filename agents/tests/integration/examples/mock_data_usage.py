...
def basic_mock_data_example():
    ...
    print(
        f"Generated terrain data with {len(terrain_response.hill_metrics.elevation.grid)}x{len(terrain_response.hill_metrics.elevation.grid[0])} grid"
    )
    ...
    print(
        f"Generated {len(equipment_response.lifts)} lifts and {len(equipment_response.trails)} trails"
    )
    print(
        f"Operational lifts: {equipment_response.operational_lifts}/{equipment_response.total_lifts}"
    )
...
def scenario_based_testing_example():
    ...
    print(
        f"Extreme weather - Temperature: {weather_response.current.temperature_c:.1f}°C"
    )
    print(f"Extreme weather - Wind: {weather_response.current.wind.speed_kmh:.1f} km/h")
    print(
        f"Extreme weather - Visibility: {weather_response.current.visibility.distance_km:.1f} km"
    )
    ...
    print(
        f"Equipment failures - Failed lifts: {failed_lifts}/{len(equipment_response.lifts)}"
    )
...
def comprehensive_testing_workflow():
    ...
    config = MockDataConfig(seed=42, grid_size=GridSize.MEDIUM, include_fis_data=True)
    ...
    print(
        f"  Terrain: {len(terrain.hill_metrics.elevation.grid)}x{len(terrain.hill_metrics.elevation.grid[0])} grid"
    )
    ...
    cache_keys = [f"terrain_{ski_area}", f"weather_{ski_area}", f"equipment_{ski_area}"]
