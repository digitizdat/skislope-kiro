...
class TestIntegrationPatterns:
    ...
    async def test_ci_cd_integration(self):
        ...
        async with WorkflowSimulator(config, agent_manager) as simulator:
            ...
            for workflow_type in critical_workflows:
                ...
                if not result.success and workflow_type == WorkflowType.TERRAIN_LOADING:
                    pytest.fail(
                        f"Critical workflow {workflow_type.value} failed: {result.error.message}"
                    )
        ...
        assert test_results.summary.success_rate >= 66.0, (
            "CI/CD requires at least 66% success rate"
        )
        assert test_results.summary.duration < 60.0, (
            "CI/CD tests must complete within 60 seconds"
        )

        print(
            f"CI/CD Results: {test_results.summary.success_rate:.1f}% success in {test_results.summary.duration:.3f}s"
        )
    ...
    async def test_monitoring_integration(self):
        ...
        avg_duration = sum(metrics_data["workflow_durations"]) / len(
            metrics_data["workflow_durations"]
        )
            ...
            alerts.append(
                {
                    ...
                    "value": success_rate,
                    "threshold": monitoring_config["alert_thresholds"][
                        "min_success_rate"
                    ],
                }
            )
        ...
        for alert in alerts:
            print(
                f"  🚨 {alert['type']}: {alert['value']:.1f} (threshold: {alert['threshold']:.1f})"
            )
    ...
    async def test_regression_testing_integration(self):
        ...
        async with WorkflowSimulator(config, agent_manager) as simulator:
            for workflow_type, baseline in baseline_data.items():
                ...
                regression_results[workflow_type] = {
                    ...
                    "regression": duration_ratio > 1.2
                    or steps_ratio < 0.8,  # 20% tolerance
                }

                status = (
                    "🔴" if regression_results[workflow_type]["regression"] else "🟢"
                )
                print(f"{status} {workflow_type.value}:")
                print(
                    f"  Duration: {result.duration:.3f}s (baseline: {baseline['duration']:.3f}s)"
                )
                print(f"  Steps: {result.steps_completed}/{baseline['steps']}")

        # Check for regressions
        regressions = [
            wf for wf, data in regression_results.items() if data["regression"]
        ]
        ...
        if regressions:
            ...
            pytest.fail(
                f"Performance regressions detected: {'; '.join(regression_details)}"
            )
    ...
    async def test_load_testing_integration(self):
        ...
        async with WorkflowSimulator(config, agent_manager) as simulator:
            ...
            assert success_rate >= 95.0, (
                f"Load test requires 95% success rate, got {success_rate:.1f}%"
            )
            assert avg_duration < 2.0, (
                f"Load test requires avg duration < 2s, got {avg_duration:.3f}s"
            )
...
async def generate_test_report():
    ...
    async with WorkflowSimulator(config, agent_manager) as simulator:
        ...
        for workflow_name, workflow_data in report_data["workflow_results"].items():
            ...
            print(
                f"  {status} {workflow_name}: {workflow_data['duration']:.3f}s "
