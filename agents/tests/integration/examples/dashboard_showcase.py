...
from ..dashboard import TestResultsDashboard, DashboardData
...
def demonstrate_dashboard_data_management():
    ...
    for i, rate in enumerate(success_rates):
        ...
        print(f"   📈 Run {i+1}: {rate}% success rate, {results.summary.duration:.1f}s duration")
...
def demonstrate_report_generation(dashboard: TestResultsDashboard):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"      • Current results: {'Available' if export_data.get('current_results') else 'None'}")
        print(f"      • Performance trends: {len(export_data.get('performance_trends', []))} points")
        print(f"      • Historical summary: {'Available' if export_data.get('historical_summary') else 'None'}")
...
def demonstrate_performance_analysis(dashboard_data: DashboardData):
    ...
    for i, trend in enumerate(recent_trends):
        ...
        print(f"   • {timestamp.strftime('%H:%M:%S')}: {trend['success_rate']:.1f}% success, {trend['duration']:.1f}s")
...
def demonstrate_agent_health_monitoring(dashboard: TestResultsDashboard):
    ...
    # Health summary
    healthy_count = sum(1 for agent in current_results.agent_health if agent.status == "healthy")
    degraded_count = sum(1 for agent in current_results.agent_health if agent.status == "degraded")
    failed_count = sum(1 for agent in current_results.agent_health if agent.status == "failed")
    ...
    overall_health = "Excellent" if failed_count == 0 and degraded_count == 0 else \
                    "Good" if failed_count == 0 else "Poor"
...
def main():
    ...
    try:
        ...
        print(f"   ✅ Multi-run test result tracking")
        ...
        print(f"\n💡 Dashboard Benefits:")
        print(f"   • Real-time visibility into test health")
        print(f"   • Historical performance tracking")
        print(f"   • Early detection of performance degradation")
        print(f"   • Comprehensive diagnostic information")
        print(f"   • Multiple output formats for different audiences")
