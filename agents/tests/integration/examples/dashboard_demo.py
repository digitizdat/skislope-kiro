...
def create_realistic_test_results(
    run_number: int = 1, base_time: datetime | None = None
) -> TestResults:
    ...
    agents_health = [
        ...
        AgentHealthStatus(
            ...
            available_methods=["get_current", "get_forecast"]
            if run_number % 3 != 0
            else ["get_current"],
...
async def simulate_continuous_testing(
    dashboard: TestResultsDashboard, num_runs: int = 10
):
...
def print_dashboard_info(dashboard: TestResultsDashboard):
    ...
    if current:
        print("\n🔍 Latest Test Run:")
        print(
            f"   • Tests: {current.summary.total_tests} total, {current.summary.passed} passed, {current.summary.failed} failed"
        )
...
async def demonstrate_live_dashboard():
    ...
    try:
        ...
        print("   ✅ Real-time test result display")
        print("   ✅ Performance trend tracking")
        print("   ✅ Agent health monitoring")
        print("   ✅ Environment issue reporting")
        print("   ✅ Interactive web interface")
        print("   ✅ Historical data analysis")
        print("   ✅ Static report generation")
        print("   ✅ Data export functionality")

        print(f"\n🌐 Dashboard is running at: {url}")
        print("📱 Features available in the web interface:")
        print("   • Summary cards with key metrics")
        print("   • Success rate and duration trend charts")
        print("   • Test category breakdown with visual indicators")
        print("   • Agent health status with response times")
        print("   • Environment issues with severity levels")
        print("   • Auto-refresh toggle for live updates")
...
def demonstrate_dashboard_api():
    ...
    print(
        f"   • Historical summary: {'Available' if data['historical_summary'] else 'None'}"
    )
    ...
    if data["current_results"]:
        ...
        print(
            f"   • Summary: {current['summary']['total_tests']} tests, {current['summary']['success_rate']:.1f}% success"
        )
        print(f"   • Categories: {len(current['categories'])}")
