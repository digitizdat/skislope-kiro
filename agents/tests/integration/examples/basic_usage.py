...
from ..workflow_simulator import WorkflowSimulator, WorkflowType, SimulationState
    ...
    async def test_multiple_workflows_batch(self):
        ...
        async with WorkflowSimulator(config, agent_manager) as simulator:
            ...
            for workflow_type in WorkflowType:
                ...
                print(f"{workflow_type.value}: {result.state.value} "
...
async def workflow_comparison_example():
    ...
    async with WorkflowSimulator(config, agent_manager) as simulator:
        for workflow_type in workflows_to_test:
            ...
            print(f"  {status} {result.duration:.3f}s ({result.steps_completed}/{result.steps_total})")
