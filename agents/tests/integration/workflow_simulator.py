...
class MockTerrainData:
    ...
    @classmethod
    def create_sample(
        cls, ski_area: str = "test_area", grid_size: tuple[int, int] = (32, 32)
    ) -> "MockTerrainData":
...
class WorkflowSimulator:
    ...
    def __init__(
        ...
        self.mock_cache_dir: Path | None = None
    ...
    async def simulate_workflow(
        self, workflow_type: WorkflowType, context: dict[str, Any] | None = None
    ) -> WorkflowResult:
        """
        ...
        try:
            ...
            self.logger.error(
                "Workflow simulation failed with exception",
                workflow=workflow_type.value,
                error=str(e),
                exc_info=True,
            )
    ...
    async def _execute_workflow_step(
        ...
        # Use workflow_context directly and update it with step context
        workflow_context.update(step.context)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "result" in response_data:
                    ...
                    self.logger.error(
                        f"Agent returned error: {response_data.get('error', 'Unknown error')}"
                    )
                    return False
            else:
                self.logger.error(f"HTTP error: {response.status_code}")
                # Fallback to mock data for HTTP errors
    ...
    async def _step_process_elevation_data(self, context: dict[str, Any]) -> bool:
        ...
        self.logger.debug(
            f"Processed terrain data: {rows}x{cols} grid, elevation range: {min_elevation}-{max_elevation}"
        )
        return True
    ...
    async def _step_validate_terrain_mesh(self, context: dict[str, Any]) -> bool:
        ...
        self.logger.debug(
            f"Validated terrain mesh: {expected_vertices} vertices, {expected_triangles} triangles"
        )
        return True
    ...
    async def _step_validate_cache_integrity(self, context: dict[str, Any]) -> bool:
        ...
        for field_name in required_fields:
            if field_name not in retrieved_data:
                return False
    ...
    async def _step_test_agent_unavailable(self, context: dict[str, Any]) -> bool:
        ...
        try:
            await self.http_client.get("http://localhost:9999/health")
            return False  # Should have failed
        except Exception:
    ...
    async def _step_baseline_performance(self, context: dict[str, Any]) -> bool:
        ...
        MockTerrainData.create_sample("baseline_test", (64, 64))
    ...
    async def _step_test_large_terrain_loading(self, context: dict[str, Any]) -> bool:
        ...
        MockTerrainData.create_sample("large_test", grid_size)
    ...
    async def _step_test_memory_usage(self, context: dict[str, Any]) -> bool:
        ...
        MockTerrainData.create_sample("memory_test", (128, 128))
    ...
    async def _step_validate_response_times(self, context: dict[str, Any]) -> bool:
        ...
        # Define performance requirements (adjusted for simulation)
        max_baseline_time = 2.0  # 2 seconds for baseline (includes sleep + overhead)
        max_large_terrain_time = 10.0  # 10 seconds for large terrain
        max_concurrent_avg_time = 1.0  # 1 second average for concurrent
    ...
    def create_test_results(
        self, workflow_results: dict[WorkflowType, WorkflowResult]
    ) -> list[TestResult]:
        """
