...
class TestJSONRPCAPIContracts:
    ...
    def _make_jsonrpc_request(
        self, client: TestClient, method: str, params: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        ...
        response = client.post(
            "/jsonrpc", json=request_data, headers={"Content-Type": "application/json"}
        )
    ...
    def test_method_not_found_error(self, hill_metrics_client):
        ...
        response = self._make_jsonrpc_request(
            hill_metrics_client, "nonExistentMethod", {}
        )
    ...
    def test_invalid_params_error(self, hill_metrics_client):
        ...
        response = self._make_jsonrpc_request(
            hill_metrics_client, "getHillMetrics", {"invalid_param": "test"}
        )
...
class TestEndToEndIntegration:
    ...
    def test_terrain_loading_workflow(self, all_clients):
        ...
        hill_response = self._make_jsonrpc_request(
            all_clients["hill_metrics"], "getHillMetrics", hill_metrics_params
        )
        ...
        weather_response = self._make_jsonrpc_request(
            all_clients["weather"], "getWeatherData", weather_params
        )
    ...
    def _make_jsonrpc_request(
        self, client: TestClient, method: str, params: dict[str, Any]
    ) -> dict[str, Any]:
        """Make a JSON-RPC request and return the response."""
        request_data = {
            "jsonrpc": "2.0",
...
class TestAgentMethodRegistry:
            ...
            for method in expected_methods:
                ...
                if "error" in response:
                    ...
                    assert error_code != -32601, (
                        f"Method '{method}' not found in {agent_name} agent"
                    )
    ...
    def _make_jsonrpc_request(
        ...
        return (
            ...
            else {"error": {"code": response.status_code}}
        )
