...
class WindData(BaseModel):
    ...
    gust_speed_kmh: float | None = Field(
        None, ge=0, description="Wind gust speed in km/h"
    )
...
class SnowData(BaseModel):
    ...
    density_kg_m3: float | None = Field(None, ge=0, description="Snow density in kg/m³")
    last_snowfall_hours: int | None = Field(
        None, ge=0, description="Hours since last snowfall"
    )
...
class VisibilityData(BaseModel):
    ...
    distance_km: float = Field(
        ..., ge=0, description="Visibility distance in kilometers"
    )
...
class WeatherData(BaseModel):
    ...
    snow: SnowData | None = Field(None, description="Snow information")
    visibility: VisibilityData = Field(..., description="Visibility information")
    uv_index: float | None = Field(None, ge=0, description="UV index")
...
class WeatherForecast(BaseModel):
    ...
    precipitation_probability: float = Field(
        ..., ge=0, le=1, description="Precipitation probability (0-1)"
    )
    ...
    snow_depth_cm: float | None = Field(
        None, ge=0, description="Expected snow depth in cm"
    )
...
class HistoricalWeatherData(BaseModel):
    ...
    precipitation_mm: float = Field(
        default=0, ge=0, description="Total precipitation in mm"
    )
    snow_depth_cm: float | None = Field(None, ge=0, description="Snow depth in cm")
    wind_speed_avg_kmh: float = Field(
        ..., ge=0, description="Average wind speed in km/h"
    )
...
class WeatherRequest(BaseModel):
    ...
    timestamp: datetime | None = Field(
        None, description="Specific timestamp (for historical data)"
    )
    include_forecast: bool = Field(default=False, description="Include forecast data")
    forecast_days: int = Field(
        default=7, ge=1, le=14, description="Number of forecast days"
    )
...
class WeatherResponse(BaseModel):
    ...
    forecast: list[WeatherForecast] = Field(
        default_factory=list, description="Weather forecast"
    )
    historical: list[HistoricalWeatherData] = Field(
...
class SkiConditions(BaseModel):
    ...
    overall_rating: float = Field(
        ..., ge=0, le=10, description="Overall ski conditions rating (0-10)"
    )
    snow_quality: SnowCondition = Field(..., description="Snow quality")
    visibility_rating: float = Field(
        ..., ge=0, le=10, description="Visibility rating (0-10)"
    )
    wind_rating: float = Field(
        ..., ge=0, le=10, description="Wind conditions rating (0-10)"
    )
    temperature_rating: float = Field(
        ..., ge=0, le=10, description="Temperature rating (0-10)"
    )
