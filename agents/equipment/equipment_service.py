...
class EquipmentService:
    ...
    def __init__(self):
        ...
        self.ski_areas = {
            ...
            "whistler": {
                "name": "Whistler",
                "bounds": {"north": 50.15, "south": 50.05, "east": -122.85, "west": -122.95},
                "base_elevation": 675,
                "top_elevation": 2284,
            },
            "st_anton": {
                "name": "Saint Anton am Arlberg",
                "bounds": {"north": 47.15, "south": 47.05, "east": 10.30, "west": 10.20},
                "base_elevation": 1304,
                "top_elevation": 2811,
            },
            "zermatt": {
            ...
            "copper_mountain": {
                ...
                "bounds": {"north": 39.55, "south": 39.45, "east": -106.10, "west": -106.20},
                "base_elevation": 2926,
                "top_elevation": 3962,
            },
        }
    ...
    async def get_equipment_data(self, request: EquipmentRequest) -> dict:
        ...
        try:
            ...
            if not (validate_coordinates(request.north, request.east) and
                    validate_coordinates(request.south, request.west)):
            ...
            if request.include_safety_equipment:
                safety_equipment = await self._generate_safety_equipment(request, ski_area)
            
            # Apply filters
            if request.operational_only:
                lifts = [lift for lift in lifts if lift.status == LiftStatus.OPERATIONAL]
            ...
            response_data = {
                ...
                "safety_equipment": [equipment.model_dump() for equipment in safety_equipment],
                "bounds": {
                    "north": request.north,
                    "south": request.south,
                    "east": request.east,
                ...
                "operational_lifts": len([l for l in lifts if l.status == LiftStatus.OPERATIONAL]),
    ...
    def _identify_ski_area(self, request: EquipmentRequest) -> Optional[dict]:
        """
        ...
        """
        for area_id, area_info in self.ski_areas.items():
            bounds = area_info["bounds"]
            
            # Check if request bounds overlap with ski area bounds
            if (request.south <= bounds["north"] and request.north >= bounds["south"] and
                request.west <= bounds["east"] and request.east >= bounds["west"]):
                return area_info
    ...
    async def _generate_lifts(
        ...
        area_size = abs(request.north - request.south) * abs(request.east - request.west)
        ...
        for i in range(num_lifts):
            ...
            # Top station is typically higher up the mountain
            top_lat = base_lat + random.uniform(-0.01, 0.01)
            top_lng = base_lng + random.uniform(-0.01, 0.01)
            ...
            lift = LiftInfo(
                ...
                heated_seats=lift_type in [LiftType.CHAIRLIFT, LiftType.GONDOLA] and random.random() > 0.5,
    ...
    async def _generate_trails(
        ...
        for i in range(num_trails):
            ...
            trail = TrailInfo(
                ...
                snow_depth_cm=random.uniform(20, 150) if random.random() > 0.1 else None,
                surface_condition=random.choice([
                    "Packed powder", "Machine groomed", "Powder", "Hard pack", "Icy patches"
                ]),
                access_lifts=[f"lift_{random.randint(1, 10):03d}" for _ in range(random.randint(1, 3))],
                connected_trails=[f"trail_{random.randint(1, num_trails):03d}" for _ in range(random.randint(0, 2))],
            )
    ...
    async def _generate_facilities(
        ...
        for i in range(num_facilities):
            ...
            amenities_map = {
                ...
                FacilityType.RESTAURANT: ["Full menu", "Alcohol service", "Reservations"],
                ...
                FacilityType.SKI_SCHOOL: ["Group lessons", "Private lessons", "Kids programs"],
                FacilityType.FIRST_AID: ["Emergency care", "Ski patrol", "Medical supplies"],
                FacilityType.PARKING: ["Day parking", "Season passes", "Shuttle service"],
    ...
    async def _generate_safety_equipment(
        ...
        for i in range(num_equipment):
            ...
            equipment = SafetyEquipment(
                ...
                installation_date=datetime.now() - timedelta(days=random.randint(30, 3650)),
                associated_trail=f"trail_{random.randint(1, 20):03d}" if random.random() > 0.5 else None,
                associated_lift=f"lift_{random.randint(1, 10):03d}" if random.random() > 0.7 else None,
            )
