...
class ElevationData(BaseModel):
    ...
    resolution: float = Field(..., gt=0, description="Grid resolution in meters per cell")
    bounds: GeographicBounds = Field(..., description="Geographic bounds of the data")
    no_data_value: float = Field(default=-9999, description="Value representing no data")
...
class AspectData(BaseModel):
    ...
    grid: List[List[float]] = Field(..., description="2D aspect grid in degrees (0-360)")
    resolution: float = Field(..., gt=0, description="Grid resolution in meters per cell")
    bounds: GeographicBounds = Field(..., description="Geographic bounds of the data")
...
class SafetyZone(BaseModel):
    ...
    material: Optional[str] = Field(None, description="Zone material type")
...
class CourseMarker(BaseModel):
    ...
    orientation: float = Field(default=0, ge=0, lt=360, description="Marker orientation in degrees")
...
class HillMetrics(BaseModel):
    ...
    surface_classification: SurfaceClassification = Field(..., description="Surface type classification")
    safety_zones: List[SafetyZone] = Field(default_factory=list, description="Safety zones for FIS compatibility")
    course_markers: List[CourseMarker] = Field(default_factory=list, description="Course markers for FIS compatibility")
...
class TerrainRequest(BaseModel):
    ...
    bounds: GeographicBounds = Field(..., description="Geographic bounds for data extraction")
    grid_size: GridSize = Field(default=GridSize.MEDIUM, description="Desired grid resolution")
    include_surface_classification: bool = Field(default=True, description="Include surface type analysis")
    include_safety_zones: bool = Field(default=False, description="Include FIS safety zones")
    include_course_markers: bool = Field(default=False, description="Include FIS course markers")
