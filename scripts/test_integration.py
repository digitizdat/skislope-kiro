    ...
    async def run_python_integration_tests(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-m", "pytest", "agents/tests/test_integration.py", "-v"],
                cwd=project_root,
                capture_output=True,
                text=True
            )
            ...
            if result.returncode == 0:
                ...
                logger.error("Python integration tests failed", return_code=result.returncode)
                return False
    ...
    async def run_frontend_integration_tests(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error("Frontend integration tests failed", return_code=result.returncode)
                return False
            ...
            async with httpx.AsyncClient() as client:
                for endpoint, methods in expected_methods.items():
                    for method in methods:
                        ...
                        if response.status_code != 200:
                            logger.error(f"HTTP error for {method} at {endpoint}", status=response.status_code)
                            return False
