...
class AgentTester:
    ...
    async def test_single_agent(self, agent_id: str, config: Dict) -> Dict:
        ...
        all_tests_passed = all(
            test_result["success"]
            for test_result in agent_results["tests"].values()
        )
...
def print_summary(results: Dict):
    ...
    successful_agents = sum(
        1 for agent_result in results["agents"].values()
    ...
    for agent_id, agent_result in results["agents"].items():
        ...
        for test_name, test_result in agent_result["tests"].items():
            ...
            response_time = f"{test_result['response_time_ms']:.0f}ms" if test_result["response_time_ms"] else "N/A"
